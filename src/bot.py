    ...
    if command.startswith("newchar"):
        ...
        if meta.get('battle') != 'N/A':
            ...
            if rng <= chanceOfNothing:
                ...
                data = {'battle': monster,
                        'enemyHp': monster['health'], 'stage': 1}
                firebase.patch('/Characters/' + user + '/Meta', data)
                response = "You encountered a " + \
                    monster['name'] + " with " + \
                    str(monster['health']) + " health. Use attack or flee."
    elif command.startswith("flee"):
        stage = firebase.get('/Characters/' + user + '/Meta/stage', None)
        if stage in (1, 2, 3):
            data = {'battle': 'N/A', 'enemyHp': 'N/A', 'stage': 0}
            firebase.patch('/Characters/' + user + '/Meta', data)
            response = "You escaped safely back to " + \
                firebase.get('/Characters/' + user + '/Meta/location', None)
        else:
        ...
        critChance = character.get('Attributes').get(
            'luck') + 100 * weapon.get('crit')
        if random.randint(0, 100) <= critChance:
            heroDmg = heroDmg * weapon.get('mod')
        heroDmg = heroDmg + \
            math.floor(character.get('Attributes').get('strength') * .5)
        ...
        if character.get('Attributes').get('dexterity') >= math.floor(monster.get('power') * .5):
            ...
            if mCurHp <= 0:
                ...
                if stage == 1:
                    ...
                    firebase.patch('/Characters/' + user + '/Meta', data)
                    response = "You dealt " + heroDmg + " damage and killed the " + \
                        monster.get('name') + "! You are currently at " + health + " health. " + \
                        response + " Continue your adventure by saying 'adventure'"
                elif stage == 2:
                    # Beat stage 2
                    ...
                    data = {'battle': 'N/A', 'enemyHp': 'N/A', 'stage': 0}
                    firebase.patch('/Characters/' + user + '/Meta', data)
                    response = "You dealt " + heroDmg + " and beat " + \
                        monster.get('name') + "You are currently at " + \
                ...
                if health <= 0:
                    ...
                    response = "You dealt " + str(heroDmg) + " and received " + str(mDmg) + " damage. You are currently at " + str(
                        health) + " health. " + monster.get('name') + " has " + str(mCurHp) + " health left."
        else:
            health = health - mDmg
            if health <= 0:
                response = new_user(character.get('Name'), user)
                ...
                if mCurHp <= 0:
                    ...
                    firebase.patch('/Characters/' + user + '/Attributes', data)
                    # Add critical hit flavor
                    response = "You dealt " + heroDmg + " and received " + mDmg + " damage. You are currently at " + \
                        health + " health. " + \
                        monster.get('name') + " has " + \
                        mCurHp + " health left."
...
def new_user(name, user):
    data = {'Name': name, 'Meta': {'level': 1, 'exp': 0, 'money': 0, 'battle': 'N/A', 'enemyHp': 'N/A', 'stage': 0, 'location': 'Dire Village', 'trader': 'general manager'}, 'armor': 'naked',
            'Attributes': {'charisma': 0, 'dexterity': 0, 'health': 10, 'intelligence': 0, 'luck': 0, 'strength': 0, 'AllocationPoints': 5, 'maxhp': 10}, 'Inventory': {'soylent': 1}, 'weapon': 'fists'}
